        ...
        writev(chunks, cb) {
            ...
            chunks.forEach(item => {
                ...
                if (!body.time || !body.host)
                    throw new Error("Timetsamps  or Hostname are not enables in pino logger instance!");
        ...
        write(body, enc, cb) {
            ...
            if (gzip) {
                zlib.gzip(value, (c_err, buffer) => {
                    if (!c_err) {
                        if (opts.includeLevels.indexOf(levelInt[body.level]) != -1) {
                            redis.zadd(redisKey, body.time, buffer.toString("binary"), (err, result) => {
                                if (!err) {
                                    splitter.emit("insert", body);
                                }
                                else {
